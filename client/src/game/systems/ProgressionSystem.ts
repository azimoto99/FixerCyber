...
export class ProgressionSystem extends EventEmitter {
  ...
  loadCharacterFromSave(
    ...
    this.activeCharacters.set(playerId, character);
    console.log(
      `🏠 Character loaded from safehouse: ${saveData.characterName}`
    );
  ...
  handleCharacterDeath(
    ...
    console.log(
      `💀 ${character.characterName} died! Lost ${creditsLost} credits and ${character.weapons.length + character.armor.length + character.inventory.length} items`
    );
  ...
  saveProgressAtSafehouse(playerId: string): boolean {
    ...
    if (!character.ownedSafehouseId) {
      console.log(
        `⚠️ ${character.characterName} tried to save but doesn't own a safehouse`
      );
      return false;
    }
    ...
    if (distance > 50) {
      // Must be within 50 units of safehouse
      console.log(
        `⚠️ ${character.characterName} is too far from safehouse to save (${Math.floor(distance)} units away)`
      );
      return false;
    }
    ...
    console.log(
      `💾 ${character.characterName} saved progress at safehouse with ${character.currentCredits} credits`
    );
  ...
  purchaseSafehouse(playerId: string, safehouseId: string): boolean {
    ...
    if (character.currentCredits < safehouse.cost) {
      console.log(
        `⚠️ ${character.characterName} needs ${safehouse.cost - character.currentCredits} more credits for safehouse`
      );
      return false;
    }
    ...
    console.log(
      `🏠 ${character.characterName} purchased safehouse in ${safehouse.district} for ${safehouse.cost} credits`
    );
  ...
  awardCredits(playerId: string, amount: number, reason: string): boolean {
    ...
    console.log(
      `💰 ${character.characterName} earned ${amount} credits for ${reason} (total: ${character.currentCredits})`
    );
  ...
  awardKill(playerId: string): boolean {
    ...
    console.log(
      `🎯 ${character.characterName} eliminated a player (${character.killCount} total kills)`
    );
  ...
  completeContract(playerId: string, contractValue: number): boolean {
    ...
    console.log(
      `📋 ${character.characterName} completed contract #${character.contractsCompleted}`
    );
    return true;
  }
  ...
  recordSuccessfulHack(playerId: string, hackReward: number): boolean {
    ...
    console.log(
      `💻 ${character.characterName} successfully hacked target (${character.hacksSuccessful} total hacks)`
    );
    return true;
  }
  ...
  private releaseSafehouse(safehouseId: string): void {
    ...
    if (safehouse) {
      ...
      console.log(
        `🏠 Safehouse in ${safehouse.district} is now available for purchase`
      );
    }
  }
  ...
  private loadPlayerSaves(): void {
    try {
      ...
      if (stored) {
        ...
        console.log(
          `💾 Loaded ${this.playerSaves.size} player saves from storage`
        );
      }
    } catch (error) {
