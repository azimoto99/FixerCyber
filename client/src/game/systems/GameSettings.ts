...
export class GameSettingsManager {
  ...
  saveSettings(): void {
    try {
      localStorage.setItem(
        GameSettingsManager.STORAGE_KEY,
  ...
  getEffectiveZoom(
    ...
    if (
      !this.settings.enforceZoomFairness &&
      this.settings.manualZoom !== undefined
    ) {
  ...
  getValidationWarnings(
    ...
    if (!this.isConfigurationFair(canvasWidth, canvasHeight, zoom)) {
      warnings.push(
        'Current display configuration may provide unfair advantage in multiplayer'
      );
    }

    if (!this.settings.enforceZoomFairness && !this.settings.competitiveMode) {
      warnings.push(
        'Zoom fairness is disabled - this may impact competitive balance'
      );
    }
    ...
    if (aspectRatio > 2.5) {
      warnings.push(
        'Ultra-wide display detected - consider enabling competitive mode for fair play'
      );
    }
