...
export class BuildingInteractionSystem {
  ...
  private checkDoorProximity(playerPosition: { x: number; y: number }) {
    ...
    if (doors.length > 0) {
      ...
      const closest = doors.reduce((prev, curr) => {
        const prevDist = this.distanceToPoint(playerPosition, {
          x: prev.worldX,
          y: prev.worldY,
        });
        const currDist = this.distanceToPoint(playerPosition, {
          x: curr.worldX,
          y: curr.worldY,
        });
  ...
  private getDoorsNearPlayer(playerPosition: {
    x: number;
    y: number;
  }): DoorInfo[] {
    ...
    for (let dx = -1; dx <= 1; dx++) {
      for (let dy = -1; dy <= 1; dy++) {
        const chunk = this.worldSystem?.getChunk(
          `chunk_${chunkX + dx}_${chunkY + dy}`
        );
        if (
          chunk?.generatedData?.doors &&
          Array.isArray(chunk.generatedData.doors)
        ) {
          // Filter doors to avoid duplicates and only get nearby ones
          const chunkDoors = chunk.generatedData.doors.filter(
            (door: DoorInfo) => {
              const distance = this.distanceToPoint(playerPosition, {
                x: door.worldX,
                y: door.worldY,
  ...
  private getInteriorSize(buildingType: string): {
    width: number;
    height: number;
  } {
    const sizes = {
    ...
    return (
      sizes[buildingType as keyof typeof sizes] || { width: 12, height: 10 }
    );
  }
