...
export class LootSystem extends EventEmitter {
  ...
  private readonly LOOT_TABLES = {
    ...
    building_search: { common: 70, uncommon: 20, rare: 7, epic: 2.5, legendary: 0.5 },
    contract_reward: { common: 30, uncommon: 40, rare: 20, epic: 8, legendary: 2 },
  ...
  generateContractReward(position: Vector2, contractDifficulty: number): LootDrop[] {
    ...
    const creditsDrop = this.createLootDrop(creditsItem, position, lootCount * 40)
  ...
  pickupLoot(lootId: string, playerId: string): LootItem | null {
    ...
    console.log(`📦 ${playerId} picked up ${lootDrop.item.name} (${lootDrop.item.rarity})`)
  ...
  private rollRarity(source: keyof typeof this.LOOT_TABLES): Rarity {
    ...
    if (roll < table.legendary + table.epic + table.rare + table.uncommon) return 'uncommon'
    return 'common'
  }
  ...
  private getRandomLootCount(source: keyof typeof this.LOOT_TABLES): number {
    ...
    switch (source) {
      case 'enemy_kill': return Math.random() < 0.7 ? 1 : 2
      case 'building_search': return Math.random() < 0.5 ? 1 : Math.random() < 0.8 ? 2 : 3
      case 'contract_reward': return 2 + Math.floor(Math.random() * 2) // 2-3 items
      case 'boss_kill': return 3 + Math.floor(Math.random() * 3) // 3-5 items
      case 'rare_chest': return 4 + Math.floor(Math.random() * 3) // 4-6 items
      default: return 1
    }
  }
  ...
  private generateWeapon(rarity: Rarity): WeaponLoot {
    ...
    const weapon: WeaponLoot = {
      ...
      accuracy: Math.min(100, Math.floor(randomize(template.accuracy) * multiplier)),
      ...
      value: Math.floor(template.value * multiplier * (0.8 + Math.random() * 0.4)),
      stackable: false,
      maxStack: 1,
      description: this.generateWeaponDescription(weaponType),
      iconColor: this.getRarityColor(rarity),
      specialEffect: rarity === 'epic' || rarity === 'legendary' ? this.rollSpecialEffect(rarity) : undefined
    }
  ...
  private generateArmor(rarity: Rarity): ArmorLoot {
    ...
    const armor: ArmorLoot = {
      ...
      durability: Math.floor(randomize(template.durability) * multiplier),
      resistances: {
        kinetic: Math.floor(randomize(template.resistances.kinetic) * multiplier),
  ...
  private generateItem(rarity: Rarity): ItemLoot {
    ...
    if (rarity === 'epic' || rarity === 'legendary') {
      itemType = ['augment_chip', 'hack_tool'][Math.floor(Math.random() * 2)] as ItemType
    }
  ...
  private createLootDrop(item: LootItem, position: Vector2, angleOffset: number = 0): LootDrop {
    ...
    console.log(`✨ ${item.rarity} ${item.name} dropped at (${Math.floor(position.x)}, ${Math.floor(position.y)})`)
  ...
  private updateLootDrops(deltaTime: number) {
    this.activeLoot.forEach(drop => {
      ...
      if (drop.item.rarity !== 'common') {
        drop.glowIntensity = this.getRarityGlow(drop.item.rarity) * (0.7 + 0.3 * Math.sin(drop.floatOffset * 2))
      }
    })
  }
  ...
  private rollSpecialEffect(rarity: Rarity): string {
    const effects = {
      ...
      legendary: ['Chain Lightning', 'Explosive Rounds', 'Phase Shift', 'Auto-Targeting', 'Infinite Ammo']
    }
