...
export class LoadingSystem {
  ...
  async startLoading(
    ...
    try {
      console.log(`🌍 Starting world generation for ${radius} chunk radius around (${spawnPosition.x}, ${spawnPosition.y})`)
      ...
      this.updateProgress('generating', 0, `Generating ${this.loadingProgress.totalChunks} world chunks...`)
      ...
      console.log(`✅ World generation complete! Generated ${this.loadingProgress.chunksLoaded} chunks in ${((performance.now() - this.loadingStartTime) / 1000).toFixed(1)}s`)
  ...
  private async generateChunksWithProgress(chunks: { x: number, y: number }[]): Promise<void> {
    ...
    for (let i = 0; i < chunks.length; i += batchSize) {
      ...
      for (const { x, y } of batch) {
        try {
          ...
          this.updateProgress('generating', 0, `Generating chunk (${x}, ${y})...`)
          ...
          if (chunk) {
            ...
            console.log(`📦 Generated chunk (${x}, ${y}) in ${chunkGenTime.toFixed(1)}ms`)
          }

          // Update progress
          const progress = (this.loadingProgress.chunksLoaded / this.loadingProgress.totalChunks) * 90 // 90% for chunk generation
          this.updateProgress('generating', progress, `Generated ${this.loadingProgress.chunksLoaded}/${this.loadingProgress.totalChunks} chunks`)
  ...
  async preloadChunksAround(position: Vector2, radius: number = 1): Promise<void> {
    if (this.isLoading) {
      console.warn('LoadingSystem: Cannot preload while main loading is active')
      return
    }
    ...
    console.log(`🔄 Preloading ${chunksToGenerate.length} additional chunks...`)
  ...
  getLoadingStats() {
    ...
    return {
      ...
      chunksLoaded: this.loadingProgress.chunksLoaded,
      totalChunks: this.loadingProgress.totalChunks,
      chunksPerSecond: this.loadingProgress.chunksLoaded > 0 ? (this.loadingProgress.chunksLoaded / (elapsedTime / 1000)) : 0,
