...
export class WorldSystem extends EventEmitter {
  ...
  private updateWorldTime(deltaTime: number) {
    ...
    this.worldState.timeOfDay =
      (this.worldState.timeOfDay + deltaTime / 1000) % 24;
  }
  ...
  loadChunksAroundPosition(position: Vector2, radius: number = 2) {
    ...
    if (chunksToLoad.length > 0) {
      ...
      console.log(
        `🗺️ Loaded ${chunksToLoad.length} new chunks around (${centerChunkX}, ${centerChunkY})`
      );
    }
  ...
  isBlocked(position: Vector2): boolean {
    ...
    if (chunk) {
      ...
      if (
        collision &&
        collision[tileY] &&
        typeof collision[tileY][tileX] === 'boolean'
      ) {
  ...
  private getDistanceToBuilding(point: Vector2, building: any): number {
    ...
    const dx = Math.max(
      0,
      Math.abs(point.x - buildingCenterX) - building.size.x / 2
    );
    const dy = Math.max(
      0,
      Math.abs(point.y - buildingCenterY) - building.size.y / 2
    );
  ...
  getWorldStats() {
    ...
    chunks.forEach(chunk => {
      // Count districts
      stats.districtCounts[chunk.districtType] =
        (stats.districtCounts[chunk.districtType] || 0) + 1;
