...
export class ContractSystem extends EventEmitter {
  ...
  private createContract(fixer: any): Contract {
    ...
    const type = types[Math.floor(Math.random() * types.length)] as ContractType
  ...
  private generateContractTarget(type: ContractType) {
    const targets = {
      [ContractType.ASSASSINATION]: {
        ...
        data: { name: 'Target Player', level: Math.floor(Math.random() * 20) + 1 }
      },
      [ContractType.DATA_EXTRACTION]: {
        id: 'terminal_' + this.generateId(),
        type: 'terminal',
        position: { x: Math.random() * 2000 - 1000, y: Math.random() * 2000 - 1000 },
        data: { securityLevel: Math.floor(Math.random() * 5) + 1, dataType: 'corporate' }
      },
    ...
    return targets[type] || { id: 'unknown', type: 'unknown', position: { x: 0, y: 0 }, data: {} }
  }
  ...
  getContract(contractId: string): Contract | undefined {
    return this.activeContracts.get(contractId) || this.availableContracts.find(c => c.id === contractId)
  }
