...
export class InventorySystem extends EventEmitter {
  ...
  addItem(playerId: string, item: LootItem, quantity: number = 1): boolean {
    ...
    if (item.stackable) {
      ...
      if (existingSlot && existingSlot.item) {
        const spaceAvailable =
          existingSlot.item.maxStack - existingSlot.quantity;
    ...
    while (quantity > 0) {
      const emptySlot = inventory.find(
        slot => slot.item === null && !slot.locked
      );
      ...
      const amountToAdd = item.stackable
        ? Math.min(quantity, item.maxStack)
        : 1;
  ...
  equipItem(playerId: string, slotId: string): boolean {
    ...
    if (item.type === 'weapon') {
      ...
      switch (weapon.weaponType) {
        ...
        case 'pistol':
        case 'smg':
          equipSlot = equipment.secondaryWeapon
            ? 'primaryWeapon'
            : 'secondaryWeapon';
          break;
        case 'rifle':
  ...
  unequipItem(playerId: string, equipSlot: keyof EquippedGear): boolean {
    ...
    const inventorySlot = inventory.find(
      slot => slot.item?.id === equippedItem.id && slot.locked
    );
  ...
  hasInventorySpace(
    ...
    if (item.stackable) {
      ...
      const emptySlots = inventory.filter(
        slot => slot.item === null && !slot.locked
      ).length;
