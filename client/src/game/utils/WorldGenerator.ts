...
export class WorldGenerator {
  ...
  static generateChunk(x: number, y: number, districtType: string): any {
    ...
    // Place content
    const buildings = this.placeBuildingsInBlocks(
      cityPlan,
      districtType,
      random
    );
    ...
    const { tileMap, collisionMap, doors } = this.generateTileAndCollisionMaps(
      x,
      y,
      buildings,
      roads,
  ...
  private static generateCityPlan(
    chunkX: number,
    chunkY: number,
    districtType: string,
    random: () => number
  ...
  private static generateStreetNetwork(baseX: number, baseY: number): Street[] {
    ...
    const numMainHorizontal =
      Math.floor(this.CHUNK_SIZE / mainStreetSpacing) + 1;
    ...
    const numSecondaryHorizontal =
      Math.floor(this.CHUNK_SIZE / secondaryStreetSpacing) + 1;
    const numSecondaryVertical =
      Math.floor(this.CHUNK_SIZE / secondaryStreetSpacing) + 1;
  ...
  private static generateCityBlocks(
    streets: Street[],
    districtType: string,
    random: () => number
  ): CityBlock[] {
    ...
    streets.forEach(street => {
      ...
      const numBuildings = Math.floor(
        totalBuildingSpace / (minBuildingWidth + buildingSpacing)
      );

      for (let i = 0; i < numBuildings; i++) {
        // Calculate building position along street
        const t =
          (buildingSpacing + i * (minBuildingWidth + buildingSpacing)) /
          streetLength;
        ...
        const buildingX =
          streetX + perpX * (street.width / 2 + buildingDepth / 2) * side;
        const buildingY =
          streetY + perpY * (street.width / 2 + buildingDepth / 2) * side;

        const buildingWidth =
          minBuildingWidth + random() * (maxBuildingWidth - minBuildingWidth);
  ...
  private static placeBuildingsInBlocks(
    cityPlan: CityPlan,
    districtType: string,
    random: () => number
  ): any[] {
  ...
  private static shouldPlaceBuilding(
    _lot: BuildingLot,
    districtType: string,
    random: () => number
  ): boolean {
    ...
    const density =
      densityByDistrict[districtType as keyof typeof densityByDistrict] || 0.3;
    return random() < density;
  }

  // Create a building that fits the lot
  private static createBuildingForLot(
    lot: BuildingLot,
    districtType: string,
    random: () => number
  ): any {
    ...
    const baseHeight = this.getBuildingHeightForDistrict(
      districtType,
      type,
      random
    );
  ...
  private static getBuildingHeightForDistrict(
    districtType: string,
    buildingType: string,
    random: () => number
  ): number {
    ...
    const districtHeights =
      baseHeights[districtType as keyof typeof baseHeights];
    if (districtHeights) {
      return (
        districtHeights[buildingType as keyof typeof districtHeights] ||
        30 + random() * 40
      );
    }
  ...
  private static generateInfrastructure(
    baseX: number,
    baseY: number,
    districtType: string,
    random: () => number
  ...
  private static getStreetlightColor(
    districtType: string,
    random: () => number
  ): string {
    const colors = {
    ...
    const districtColors =
      colors[districtType as keyof typeof colors] || colors.residential;
    return districtColors[Math.floor(random() * districtColors.length)];
  }

  // Generate sign text
  private static generateSignText(
    districtType: string,
    random: () => number
  ): string {
    ...
    const texts =
      districtType === 'corporate' ? corporateTexts : residentialTexts;
  ...
  private static generateTileAndCollisionMaps(
    ...
    const tileMap: string[][] = Array.from({ length: tilesPerChunk }, () =>
      Array(tilesPerChunk).fill('concrete')
    );
    const collisionMap: boolean[][] = Array.from(
      { length: tilesPerChunk },
      () => Array(tilesPerChunk).fill(false)
    );
    ...
    for (let ty = 0; ty < tilesPerChunk; ty++) {
      for (let tx = 0; tx < tilesPerChunk; tx++) {
        ...
        if (districtType === 'residential' && Math.random() < 0.08)
          baseType = 'grass';
        if (districtType === 'industrial' && Math.random() < 0.06)
          baseType = 'dirt';
    ...
    for (let i = 0; i < buildings.length; i++) {
      ...
      const doorTileX = Math.max(
        0,
        Math.min(19, Math.floor(doorLocalX / tileSize))
      );
      const doorTileY = Math.max(
        0,
        Math.min(19, Math.floor(doorLocalY / tileSize))
      );
  ...
  private static getRarityForDistrict(
    districtType: string,
    random: () => number
  ): Rarity {
    const rarityWeights = {
    ...
    const weights = rarityWeights[
      districtType as keyof typeof rarityWeights
  ...
  private static generateLootTables(
    random: () => number,
    lootChance: number
  ): any[] {
    const tables = [];
