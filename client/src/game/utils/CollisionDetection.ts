...
export class CollisionDetection {
  ...
  // Point vs Rectangle
  static pointInRectangle(point: Vector2, rect: { position: Vector2; size: Vector2 }): boolean {
    return (
  ...
  static circleRectangleCollision(
    ...
    const closestX = Math.max(rect.position.x, Math.min(center.x, rect.position.x + rect.size.x))
    const closestY = Math.max(rect.position.y, Math.min(center.y, rect.position.y + rect.size.y))
  ...
  static lineCircleCollision(
    ...
    const t = Math.max(0, Math.min(1, 
      center.subtract(start).dot(end.subtract(start)) / (lineLength * lineLength)
    ))
  ...
  static lineRectangleCollision(
    ...
    const edges = [
      { start: rect.position, end: new Vector2(rect.position.x + rect.size.x, rect.position.y) },
      { start: new Vector2(rect.position.x + rect.size.x, rect.position.y), end: new Vector2(rect.position.x + rect.size.x, rect.position.y + rect.size.y) },
      { start: new Vector2(rect.position.x + rect.size.x, rect.position.y + rect.size.y), end: new Vector2(rect.position.x, rect.position.y + rect.size.y) },
      { start: new Vector2(rect.position.x, rect.position.y + rect.size.y), end: rect.position }
    ]
  ...
  static lineLineCollision(
    start1: Vector2, end1: Vector2,
    start2: Vector2, end2: Vector2
  ): boolean {
  ...
  static rayRectangleCollision(
    ...
    if (Math.abs(hitPoint.x - rect.position.x) < 0.1) normal = new Vector2(-1, 0)
    else if (Math.abs(hitPoint.x - (rect.position.x + rect.size.x)) < 0.1) normal = new Vector2(1, 0)
    else if (Math.abs(hitPoint.y - rect.position.y) < 0.1) normal = new Vector2(0, -1)
    else if (Math.abs(hitPoint.y - (rect.position.y + rect.size.y)) < 0.1) normal = new Vector2(0, 1)
  ...
  static getGridBounds(position: Vector2, size: Vector2, cellSize: number): {
    min: Vector2;
    max: Vector2;
  } {
  ...
  static getCollisionResponse(
    ...
    if (distance === 0) return { velocity1: object1.velocity, velocity2: object2.velocity }

    const normal = relativePosition.divide(distance)
    const relativeSpeed = relativeVelocity.dot(normal)

    if (relativeSpeed > 0) return { velocity1: object1.velocity, velocity2: object2.velocity }

    const restitution = 0.8 // Elasticity
    const impulse = -(1 + restitution) * relativeSpeed / (1 / object1.mass + 1 / object2.mass)

    const velocity1 = object1.velocity.add(normal.multiply(impulse / object1.mass))
    const velocity2 = object2.velocity.subtract(normal.multiply(impulse / object2.mass))
