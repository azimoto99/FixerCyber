...
export class NeuralProgram {
  ...
  private applyStealthEffects(player: any): void {
    ...
    player.detectionChance = Math.max(
      0,
      (player.detectionChance || 1) - this.effects.detectionReduction
    );
  ...
  private applyHackingEffects(player: any): void {
    ...
    player.bypassChance =
      (player.bypassChance || 0) + this.effects.bypassChance;
    player.heatReduction =
      (player.heatReduction || 0) + this.effects.heatReduction;
  }

  private removeHackingEffects(player: any): void {
    player.hackSpeed = Math.max(
      1,
      (player.hackSpeed || 1) - this.effects.hackSpeed
    );
  ...
  private static getDescriptionForType(type: string): string {
    ...
    return (
      descriptions[type as keyof typeof descriptions] || 'Unknown program type'
    );
  }
