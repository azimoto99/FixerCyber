...
export class GameServer {
  ...
  handleContractAccept(socketId: string, data: any) {
    ...
    this.contractService
      .acceptContract(player.id, data.contractId)
      .then(result => {
        if (result.success) {
  ...
  handleContractComplete(socketId: string, data: any) {
    ...
    this.contractService
      .completeContract(player.id, data.contractId, data.completionData)
      .then(result => {
        if (result.success) {
  ...
  private broadcastToNearbyPlayers(player: any, event: string, data: any) {
    ...
    const nearbyPlayers = Array.from(this.connectedPlayers.values()).filter(
      p => p.id !== player.id && this.isNearby(player, p)
    );
